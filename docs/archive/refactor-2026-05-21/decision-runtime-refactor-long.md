...
- `DecisionOutputVerifier` bloque claims sans event, jargon interne et voix
  invalide ;
- Phase 10C supprime `legacy/final_reply_backend.py` ; le backend concret du
  `DecisionReplyComposer` vit maintenant sous `llm/reply_decision_backend.py` ;
...
- `tests/test_llm_reply_decision_backend.py`
...
- `backend/src/fitmas/skills/heartbeat/runtime_adapter.py` convertit un
...
- `conversation_pipeline.py` n'importe plus `fitmas.final_reply` ; Phase 10B
  supprime aussi le forwarder `legacy/conversation_reply_adapter.py` ;
...
- `/api/v0/messages` vit maintenant sous
  `backend/src/fitmas/app/api/routes_messages.py` ; le wrapper root
  `api_messages.py` a ensuite ete supprime en Phase 10A ;
...
- `LLMReplyBackend` accepte une candidate summary deja user-safe et ne
...
Objectif : commencer la suppression physique de `legacy/`, pas seulement son
isolement.
...
- `api_read.py` importe `RuntimeDay` / `RuntimeWeek` depuis le read-model
  app canonique `fitmas.app.api.read_models`, sans bridge legacy et sans
  mentionner `WeeklyPlan` / `DayPlan` dans le fichier runtime controle ;
- les tests de draft PlanPatch ont ensuite ete retires avec
  `plan_patch_tools.py`, sans restaurer de registry compat
  `legacy_planning_tool_specs` ;
- le test d'architecture
  `tests/test_phase9x_legacy_delete_progress_architecture.py` interdit le
  retour de ces deux modules et de leurs imports.

Preuves locales 2026-05-20 :

```text
9X architecture + tool runtime:
27 passed
```

Lecture :

- ce slice ne shrinke pas encore fortement `conversation_pipeline.py` ;
- il retire deux fichiers legacy physiques qui ne portaient plus d'autorite
  runtime ;
- prochaine coupe : sortir ou supprimer les bridges canoniques restants dans
  `legacy/conversation_*`, en commençant par ceux sans dependance legacy.

## Statut Phase 9Y

Objectif : rendre le root `backend/src/fitmas/*.py` impossible a ignorer et
forcer une logique delete-first.

Livres :

- nouveau census exhaustif :
  `docs/ROOT-MODULE-CENSUS.md` ;
- les 128 fichiers root ont un owner cible, une action cible et un next slice ;
- actions autorisees uniquement :
  `delete`, `merge`, `move`, `keep_root_temporarily`, `entrypoint` ;
- les entrypoints root permanents sont bornes a
  `__init__.py`, `api.py`, `main.py` ;
- le test
  `tests/test_phase9y_root_module_census_architecture.py` echoue si un nouveau
  fichier root apparait sans classification ;
- la carte refuse explicitement le move-only : au moins 5 fichiers doivent etre
  marques `delete`, au moins 10 `merge`, et tous les fichiers doivent avoir une
  raison et un next slice.

Lecture :

- 9Y ne deplace pas encore les 128 fichiers ;
- il transforme le root en backlog de simplification auditable ;
- prochain chantier logique : supprimer les wrappers `sys.modules` et les root
  compat deja remplaces par packages (`tool_*`, `llm_gateway.py`,
  `telegram_scheduler.py`, `heartbeat*.py`), puis relancer les gates.

## Statut Phase 9Z

Objectif : appliquer la premiere coupe du census root sans changer de
...
- les imports pointent maintenant vers les packages canoniques :
  `fitmas.skills.heartbeat.*`, `fitmas.llm.gateway`,
  `fitmas.app.telegram.scheduler`, `fitmas.tools.*` ;
- `docs/ROOT-MODULE-CENSUS.md` descend de 128 a 119 fichiers root classes ;
- le test
  `tests/test_phase9z_root_wrapper_delete_architecture.py` interdit le retour
  de ces wrappers et de leurs imports.

Lecture :

- c'est une vraie reduction physique : moins 9 fichiers root ;
- le heartbeat reste encore une surface legacy via `skills/heartbeat`, mais il
  n'est plus cache derriere des aliases root ;
- Phase 10A a supprime les fichiers root delete simples
  (`api_messages.py`, `plan_patch_tools.py`, `state.py`) et a rehome le code
  encore utile de `plan_actions.py` et `replan_proposal.py`.
- Phase 10B a supprime `final_reply.py` et le forwarder
  `legacy/conversation_reply_adapter.py`.
- la reply conversation vit sous `llm/reply_backend.py` avec
  verifiers dedies ; la reply heartbeat vit sous
