...
- conversation : plus d'import `fitmas.final_reply` ;
...
- `/api/v0/messages` est deplace vers `app/api/routes_messages.py`, puis
  `api_messages.py` est supprime en Phase 10A ;
...
| Reply legacy | `llm/reply_decision_backend.py`, `llm/reply_backend.py`, `skills/heartbeat/reply_composer.py` | Root `final_reply.py` et pont `legacy/final_reply_backend.py` supprimes ; backend LLM reply encore a simplifier vers `DecisionReplyComposer` + `OutputVerifier` | `DecisionOutcome -> DecisionReplyComposer -> DecisionOutputVerifier` partout |
| Heartbeat legacy interne | `heartbeat.py`, `skills/heartbeat/heartbeat.py`, `skills/heartbeat/tool_loop.py`, `skills/heartbeat/reply_context.py`, `skills/heartbeat/runtime_adapter.py` | Wrappers legacy supprimes ; code historique encore dans le repo, hors chemin Telegram actif | Suppression physique apres dogfood runtime |
| Tools planning legacy | `legacy/tools_compat.py`, `plan_patch_tools.py` | supprimes physiquement | Clos Phase 9X / 10A |
...
- `final_reply.py` et `legacy/final_reply_backend.py` sont supprimes ; le
  backend actif est `llm/reply_decision_backend.py`.
...
## Phase 9X — Suppression compat legacy morte

Resultat local 2026-05-20 :

- `legacy/tools_compat.py` est supprime ;
- `legacy/weekly_plan_compat.py` est supprime ;
- `api_read.py` importe `RuntimeDay` / `RuntimeWeek` depuis
  `fitmas.app.api.read_models`, sans bridge legacy ni tokens
  `WeeklyPlan` / `DayPlan` dans le fichier runtime controle ;
- les tests de draft PlanPatch ne passent plus par
  `legacy_planning_tool_specs` ; la surface `plan_patch_tools.py` a ensuite
  ete supprimee en Phase 10A ;
- le test `tests/test_phase9x_legacy_delete_progress_architecture.py`
  interdit le retour de ces modules et imports.
...
- 9X est une coupe physique volontairement bornee ;
- elle ne touche pas au provider, au prompt, ni aux lanes dogfood ;
- elle transforme deux compat historiques vides en deletion reelle.

Suite logique :

- Phase 9Y : déplacer ou supprimer les bridges canoniques restants dans
  `legacy/conversation_*`, en priorite ceux sans dependance legacy
  (`conversation_canonical_clarification_bridge.py`, puis surfaces activity /
  readonly selon fan-in) ;
- objectif jeudi : `legacy/` ne doit plus etre un lieu permanent, seulement une
  file d'attente de suppression avec gates rouges.

## Phase 9Y — Census root delete-first

Resultat local 2026-05-20 :

- ajout de `docs/ROOT-MODULE-CENSUS.md` ;
- les 128 fichiers libres `backend/src/fitmas/*.py` sont classes par owner
  cible et action cible ;
- le census distingue explicitement `delete`, `merge`, `move`,
  `keep_root_temporarily` et `entrypoint` ;
- les entrypoints root autorises sont limites a `__init__.py`, `api.py` et
  `main.py` ;
- le test `tests/test_phase9y_root_module_census_architecture.py` interdit un
  nouveau fichier root non classe ;
- le gate impose une carte delete-first, pas un simple plan de rangement.

Lecture :

- 9Y ne supprime pas encore les 128 fichiers ;
- il rend chaque fichier root justiciable : soit il disparait, soit il fusionne,
  soit il bouge vers un bounded context, soit il est un hotspot temporaire
  nomme ;
- le prochain slice doit appliquer la premiere coupe du census, en priorite les
  wrappers `sys.modules` deja remplaces par packages canoniques.

## Phase 9Z — Suppression wrappers root `sys.modules`

Resultat local 2026-05-20 :

- 9 fichiers root supprimes :
...
- les imports ont ete rewrites vers :
  `fitmas.skills.heartbeat.*`, `fitmas.llm.gateway`,
  `fitmas.app.telegram.scheduler`, `fitmas.tools.*` ;
- le census root passe de 128 a 119 fichiers ;
- le gate `tests/test_phase9z_root_wrapper_delete_architecture.py` interdit
  leur retour.

Lecture :

- ce n'est pas seulement du rangement : les wrappers root ont disparu ;
- le legacy heartbeat reste visible comme `skills/heartbeat`, donc plus facile
  a supprimer dans un slice dedie ;
- les prochains deletes doivent viser les surfaces root encore marquees
  `delete`, sans creer de nouveaux aliases compat.

## Callers de `fitmas.final_reply`

Phase 10B a supprime le module root `fitmas.final_reply`. Les callers directs
restants doivent donc rester vides.

```text
(aucun)
```

La conversation utilise maintenant `llm/reply_backend.py`. Le heartbeat
utilise `skills/heartbeat/reply_composer.py`.
...
- `conversation_pipeline.py` passait par le bridge reply legacy ; Phase 10B a
  supprime ce forwarder et le module root `fitmas.final_reply` ;
...
- aucun caller direct de `fitmas.final_reply` ne reste ;
...
- aucun module source n'importe `fitmas.final_reply` ;
...
- `api_messages.py` est supprime ; la route vit uniquement sous
