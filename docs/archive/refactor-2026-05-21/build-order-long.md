...
- Phase 5 initiale livree localement :
  ...
  - Phase 10C a remplace l'ancien pont `legacy/final_reply_backend.py` par
...
- Phase 7 initiale livree localement :
  ...
  - `skills/heartbeat/runtime_adapter.py` mappe heartbeat legacy vers
...
- Phase 8D livree localement :
  ...
  - `/api/v0/messages` vit sous `fitmas.app.api.routes_messages` ;
    le wrapper root `fitmas.api_messages` a ensuite ete supprime en Phase 10A ;
  ...
  - le bridge heartbeat legacy a ensuite ete supprime en Phase 10D ; le runtime
    adapter actif vit sous `skills/heartbeat/runtime_adapter.py`.
  - verification ciblee :
...
- Suite logique apres 9V/9W :
  - Phase 9X livree localement :
    - suppression physique de `legacy/tools_compat.py` et
      `legacy/weekly_plan_compat.py` ;
    - `api_read.py` lit `RuntimeDay` / `RuntimeWeek` via
      `fitmas.app.api.read_models`, sans bridge legacy ni tokens
      `WeeklyPlan` / `DayPlan` dans le fichier runtime controle ;
    - les tests draft PlanPatch ont ensuite ete retires avec
      `plan_patch_tools.py` en Phase 10A ;
    - gate anti-retour :
      `tests/test_phase9x_legacy_delete_progress_architecture.py` ;
    - verification ciblee :
      `tests/test_phase9x_legacy_delete_progress_architecture.py` +
      `tests/test_tool_runtime.py` -> 27 passed.
- Suite logique apres 9X :
  - Phase 9Y livree localement :
    - `docs/ROOT-MODULE-CENSUS.md` classe les 128 fichiers root
      `backend/src/fitmas/*.py` ;
    - chaque fichier a un owner cible, une action cible et un next slice ;
    - actions autorisees :
      `delete`, `merge`, `move`, `keep_root_temporarily`, `entrypoint` ;
    - le gate
      `tests/test_phase9y_root_module_census_architecture.py` interdit tout
      nouveau fichier root non classe et impose une carte delete-first ;
    - entrypoints root permanents bornes a `__init__.py`, `api.py`, `main.py`.
- Suite logique apres 9Y :
  - Phase 9Z livree localement :
    - suppression physique de 9 wrappers root `sys.modules` :
      `heartbeat.py`, `heartbeat_evaluation.py`, `heartbeat_roles.py`,
      `llm_gateway.py`, `telegram_scheduler.py`, `tool_contract.py`,
      `tool_metrics.py`, `tool_registry.py`, `tool_runtime.py` ;
    - imports rewrites vers packages canoniques :
      `fitmas.skills.heartbeat.*`, `fitmas.llm.gateway`,
      `fitmas.app.telegram.scheduler`, `fitmas.tools.*` ;
    - `docs/ROOT-MODULE-CENSUS.md` passe de 128 a 119 fichiers root classes ;
    - gate anti-retour :
      `tests/test_phase9z_root_wrapper_delete_architecture.py`.
- Suite logique apres 9Z :
  - Phase 10A livree localement :
    - suppression physique de `api_messages.py`, `plan_patch_tools.py` et
      `state.py` ;
    - rehome utile de `plan_actions.py` vers
      `domain/planning/session_actions.py` ;
    - rehome utile de `replan_proposal.py` vers
      `tools/replan_proposal.py` ;
    - `docs/ROOT-MODULE-CENSUS.md` passe de 119 a 114 fichiers root classes ;
    - gate anti-retour :
      `tests/test_phase10a_root_delete_candidates_architecture.py`.
  - Phase 10B livree localement :
    - suppression physique de `final_reply.py` ;
    - suppression du forwarder `legacy/conversation_reply_adapter.py` ;
    - split borne de la reply conversation sous `llm/reply_backend.py`
      + verifiers dedies ;
    - split heartbeat sous `skills/heartbeat/reply_composer.py` ;
    - `docs/ROOT-MODULE-CENSUS.md` passe de 114 a 113 fichiers root classes ;
    - gate anti-retour :
      `tests/test_phase10b_final_reply_delete_architecture.py`.
  - Phase 10C livree localement :
    - suppression physique de `legacy/final_reply_backend.py` ;
    - backend concret du `DecisionReplyComposer` renomme en
      `llm/reply_decision_backend.py::LLMReplyBackend` ;
    - conversation et pending bridge importent le backend LLM explicite, plus
      un module `legacy/` ;
    - gate anti-retour :
      `tests/test_phase10c_reply_backend_legacy_delete_architecture.py`.
  - Phase 10D livree localement :
    - suppression physique de `legacy/heartbeat_runtime_adapter.py` et
      `legacy/heartbeat_skill_bridge.py` ;
    - heartbeat runtime adapter vit sous
      `skills/heartbeat/runtime_adapter.py` ;
    - API/Telegram/Ops importent le module heartbeat canonique ;
    - gate anti-retour :
      `tests/test_phase10d_heartbeat_legacy_delete_architecture.py`.
  - ensuite seulement deplacer les familles API / Telegram / planning ;
  - le cap jeudi reste : un runtime plus petit et un `legacy/` supprime, pas un
    legacy mieux etiquete ni un root simplement relooke.
...
- **Dual-source de verite runtime** : ✅ core ferme le 3 mai. `domain/planning/session_actions.py` et `mutations.py` ne mutent plus `DayPlan`; `signals.py` lit `ScheduledSession`; `activities.py` matche les activites contre `ScheduledSession`; `api_activities.py` et `strava.py` ne chargent plus le plan hebdo pour matcher ou marquer une activite.
- **Lectures `DayPlan/WeeklyPlan` restantes** : limitees au template/onboarding/admin/compat (`schema.py`, `models.py`, `repository.py`, `api_onboarding.py`, `api_read.py` fallback template de `/week`, `seed.py`, `api_debug.py`, `api_ops.py`).
...
| P1-quinquies | ✅ Lane terminale `close_turn` — clotures sociales sans tools, sans marker question ouverte, composer final migre ensuite vers `llm/reply_backend.py` | 0.5j | `docs/superpowers/plans/2026-05-05-terminal-close-lane.md` |
| P1-sexies | ✅ Composer final `no_change` — `CoachDecision(no_change)` + compat legacy passent ensuite par `llm/reply_backend.py`, avec faits memoire/execution appliques | 0.5j | `docs/superpowers/plans/2026-05-05-no-change-final-composer.md` |
| P1-septies | ✅ Composer final `plan_lookup` — lecture factuelle via `llm/reply_backend.py` avec guard anti-drift chiffres/jours/zones/statuts | 0.5j | `docs/superpowers/plans/2026-05-05-plan-lookup-final-composer.md` |
...
Fix livre :
- `domain/planning/session_actions.py` mute uniquement `ScheduledSession`.
...
Garde-fous ajoutes :
- test statique interdisant `WeeklyPlan/DayPlan` et les helpers legacy dans `domain/planning/session_actions.py`, `mutations.py`, `signals.py`, `activities.py`.
...
- refactor coherence — etat reel :
  ...
  - `domain/planning/session_actions.py` / `mutations.py` mutent uniquement `ScheduledSession` ✓
