...
| **2.0 — Contrat contexte** ✅ | context-pack en **couches** (slots `target / actuals / constraints[] / signals[]`) + bridge `fact→TypedConstraint` (santé **toujours** incluse). **Seam stable** : le moteur de contexte profond (Q5) se branche derrière sans toucher au reste. **Fait** : voie (b) forward-only (`actuals_from_week`), `signals` slot vide, cold-start `None` (`build_context_pack`). | tests unitaires bridge + pack |
| **2.1 — Générateur** ✅ | LLM génère une semaine typée depuis le pack, boucle generate→verify (max ~3, filet template). LLM-first, template filet, anti-drop contrainte-aware Tier 2, offline. Harness : `scripts/v0_eval/generate_weeks.py`. | **générer 4-6 semaines offline → vérif main vs app** + couche 2 |
| **3 — Tool runtime** | `propose_week` coach-callable → moteur → vérif → policy. **3a fait** (montre la semaine, preview non-committante, seed LLM-déclaré, generation_llm 4096) ; **3b** = confirmation → commit + store typé + chaînage. | **couche 2** live sous-agent |

Fait le 5 juin : **Slice 0, 1, 1.5, 2.0** (specs `2026-06-05-moteur-sport-*`, design `2026-06-05-slice-2-0-voie-b-design.md`). Fait le 6 juin : **Slice 2.1** (générateur LLM-first, template filet, anti-drop contrainte-aware Tier 2, offline ; harness `scripts/v0_eval/generate_weeks.py`) ; **Slice 3a** (propose_week câblé, montre la semaine en preview). Suite : **passe de simplification** (ratchet meso/+câblage) **puis Slice 3b** (confirmation → commit + store).
