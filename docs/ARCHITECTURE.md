...
- **Block_reason typed** (199a40e) : `PlanMutationService` expose `PlanBlockedMutationEvent.block_reason`. La reply utilisateur est derivee du code (`same_sport_proximity`, `occupied_training_target`), jamais improvisee par le LLM. Chaque blocage loggue un `mutation_blocked` structure. Depuis le 6 mai 2026, repos/recuperation n'est plus un hard-block writer : la coherence est jugee par la review semaine.
...
- **Recovery via review** (6 mai 2026) : les anciens guards `protected_recovery_target` sont deprecies en runtime. Un repos fait partie du plan, mais n'est plus un verrou de calendrier ; les mutations qui le deplacent ou le consomment passent par validation/review de coherence.
