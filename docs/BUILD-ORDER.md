...
| P1-octies | 🔥 Prompt/context optimization pass + `decide() returned None` reduction — reduire et specialiser le contexte donne a chaque couche LLM (`turn_planner`, `decide`, composers, verifiers, heartbeat), auditer les prompts qui diluent la decision, et mesurer/reduire les retours `None` du decisionnaire — chantier actif ; slices 1-2 prompt diet par capability livres localement | 1-2j | `docs/PROMPT-CONTEXT-REFACTOR.md` + section ci-dessous |
