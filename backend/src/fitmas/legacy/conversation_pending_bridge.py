...
from fitmas.legacy import conversation_planning_bridge, pending_reply_adapter
...
def verify_pending_accept_resolution(
    *,
    ...
    request_json_fn = request_json_fn or gw.request_json
    try:
...
def patch_was_applied(service_result: PlanPatchServiceResult | None) -> bool:
...
def applied_plan_patch_reply(service_result: PlanPatchServiceResult | None, *, fallback: str) -> str:
...
def blocked_plan_patch_reply(service_result: PlanPatchServiceResult | None) -> str:
...
def _pending_reply(
    *,
    mode: pending_reply_adapter.PendingReplyMode,
    pending_confirmation,
    pending_resolution: Any | None = None,
    user_text: str | None = None,
    decision_reply_composer: Any | None = None,
) -> str:
    return pending_reply_adapter.compose_pending_reply(
        mode=mode,
        pending_confirmation=pending_confirmation,
        pending_resolution=pending_resolution,
        user_text=user_text,
