...
from fitmas.legacy.decision import CoachUnderstanding, DecisionReplyComposer
from fitmas.legacy.decision import pending_reply
from fitmas.legacy.decision.planning_outcomes import (
...
def _decision_reply_composer() -> DecisionReplyComposer:
    backend_cls = getattr(import_module("fitmas.legacy.llm.reply_decision_backend"), "LLMReplyBackend")
    return DecisionReplyComposer(reply_backend=backend_cls())


def _apply_patch_for_user():
    return getattr(import_module("fitmas.legacy.domain.planning.patch_mutation_service"), "apply_patch_for_user")


def _default_request_json_fn():
    return getattr(import_module("fitmas.legacy.llm.gateway"), "request_json")


def _default_strong_model() -> str:
    return str(getattr(import_module("fitmas.legacy.llm.gateway"), "DEFAULT_STRONG_MODEL"))
