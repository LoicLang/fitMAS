...
from fitmas.legacy.domain.execution.activities import infer_activity_title, match_activity_to_day, normalize_activity_sport
from fitmas.legacy.domain.planning.patch_mutation_service import complete_session_from_activity_for_user, mark_session_completed_for_user
from fitmas.legacy.domain.athlete.training_load import estimate_tss
...
def import_recent_activities(
    ...
    *,
    ...
    for raw_activity in fetch_recent_activities(access_token, per_page=30):
        ...
        try:
            from fitmas.legacy.domain.athlete.threshold_estimation import update_threshold_facts
            update_threshold_facts(db, user, activity)
        except Exception:
            import logging
            logging.getLogger(__name__).exception("Threshold estimation failed (non-blocking)")
        try:
            from fitmas.legacy.domain.planning.adaptation import check_and_adapt_post_activity
