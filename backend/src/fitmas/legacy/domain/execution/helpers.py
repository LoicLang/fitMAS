...
from fitmas.legacy.domain.execution.claims import ActivityClaim, extract_claims_from_facts
...
def activities_on_local_date(db: Session, user: s.User, *, target_date: date) -> list[s.Activity]:
    ...
    for activity in execution_repo.get_activities(db, user.id, limit=120):
...
def claimed_activities_on_local_date(
    ...
    facts = memory_repo.get_active_memory_items(db, user.id, profile_limit=24, working_limit=48, total_limit=64)
...
def activities_last_days(db: Session, user: s.User, *, days: int) -> list[s.Activity]:
    ...
    timezone = get_timezone(user.timezone)
    matched: list[s.Activity] = []
    for activity in execution_repo.get_activities(db, user.id, limit=160):
        if activity.started_at is None:
            continue
        started_at = activity.started_at
        if started_at.tzinfo is None:
...
def claimed_activities_last_days(
    ...
    cutoff = get_local_now(user.timezone).date() - timedelta(days=max(0, days - 1))
    facts = memory_repo.get_active_memory_items(db, user.id, profile_limit=24, working_limit=64, total_limit=80)
    return [
        claim
